...
import com.mycompany.methotels.pages.AdminLogin;
...
// If user is authorised to the page, then all is well so allow access
            if (isAuthorised(rolesAllowed)) {
                ...
                Link pageProtectedLink
                        = pageRenderLinkSource.createPageRenderLinkWithContext(AdminLogin.class,
                                requestedPageName);
            ...
            Link loginPageLink = pageRenderLinkSource.createPageRenderLink(AdminLogin.class);
    ...
    private boolean isAuthenticated() throws IOException {
        Admin user = sessionStateManager.getIfExists(Admin.class);
        if (user != null) {
            if (user.getRola() == Role.Admin) {
                return true;
            }
        }
        return false;
